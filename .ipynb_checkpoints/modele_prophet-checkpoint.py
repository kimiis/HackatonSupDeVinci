...
#
# INSTALLATION :
#   pip install prophet matplotlib mlflow
# ==============================================================================
...
for nom, col in VARIABLES.items():
    ...
    # Chaque variable est tracée dans une run MLflow indépendante.
    # Cela permet de comparer les modèles dans l'UI MLflow.
    with mlflow.start_run(run_name=nom):

        # ── Paramètres du modèle loggués dans MLflow ──────────────────────────
        mlflow.log_params({
            "variable":          col,
            "n_observations":    len(df_p),
            "annee_debut":       int(df_p["ds"].dt.year.min()),
            "annee_fin":         int(df_p["ds"].dt.year.max()),
            "horizon_projection": 2100,
            "interval_width":    0.95,
        ...
        # freq="YS" = Year Start = 1er janvier de chaque année.
        annees_restantes = 2100 - df_p["ds"].dt.year.max()
        future = m.make_future_dataframe(periods=annees_restantes, freq="YS")

        # m.predict() retourne yhat (prédiction centrale), yhat_lower/upper (IC 95%)
        fc = m.predict(future)
        fc["annee"] = fc["ds"].dt.year

        # Retransformation log1p → expm1 pour les coûts de catastrophes
        if log_transform:
        ...
        # Sauvegarde du forecast CSV + log comme artefact MLflow
        chemin_fc = f"data/resultats/forecast_{nom}.csv"
        fc[["ds", "annee", "yhat", "yhat_lower", "yhat_upper"]].to_csv(chemin_fc, index=False)
        mlflow.log_artifact(chemin_fc)
        ...
        # Log des métriques dans MLflow (seulement si disponibles)
        mlflow_metrics = {k: v for k, v in met.items() if v is not None}
        if mlflow_metrics:
            mlflow.log_metrics(mlflow_metrics)

        # Log du modèle Prophet lui-même comme artefact MLflow
